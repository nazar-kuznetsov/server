...
router.route('/register').post(async (req, res) => {
  ...
  if (find_user) return res.status(400).json({ sucess: false, message: 'Указанный email уже зарегистрирован' });
...
router.route('/login').post(async (req, res) => {
  ...
  if (!user) return res.json({ success: false, message: 'Email не найден' });

  const correct_password = await bcrypt.compare(password, user.password);

  if (!correct_password) return res.json({ success: false, message: 'Неверный пароль' });
...
router.route('/delete').delete(auth, async (req, res) => {
  await Users.findOneAndDelete({ _id: req.body._id }, (err, result) => {
    ...
    return res.json({ success: true, message: 'Акаунт удален успешно' });
  });
...
router.route('/email-reset-password').post(async (req, res) => {
  ...
  if (user === null) {
    return res.status(400).json('E-mail не зарегестрирован');
  }
  ...
  await transporter.sendMail(email_option(email, user.name, reset_password_token), (error, info) => {
    ...
    if (error) return res.status(400).json({ sucess: false, message: error });
